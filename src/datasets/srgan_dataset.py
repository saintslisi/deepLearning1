...
from src.utils.imgproc import image_to_tensor, image_resize
...
class BaseImageDataset(Dataset):
    ...
    def __getitem__(
        ...
        if self.lr_image_file_names is not None:
            ...
            crop_size_gt = self.gt_image_size
            crop_size_lr = self.gt_image_size // self.upscale_factor

            if w_lr > crop_size_lr and h_lr > crop_size_lr:
                x_lr = random.randint(0, w_lr - crop_size_lr)
                y_lr = random.randint(0, h_lr - crop_size_lr)
                
                x_gt = x_lr * self.upscale_factor
                y_gt = y_lr * self.upscale_factor
                
                lr_image = lr_image[y_lr:y_lr + crop_size_lr, x_lr:x_lr + crop_size_lr, :]
                gt_image = gt_image[y_gt:y_gt + crop_size_gt, x_gt:x_gt + crop_size_gt, :]
